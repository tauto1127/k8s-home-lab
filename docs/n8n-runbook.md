...
n8n の **preparation** だけ。クラスタ apply、`flux resume`、手動 reconcile はしない。
merge しても起動しない。
...
1. Preparation（このPR）: 外側 `Kustomization/n8n` と内側 `HelmRelease/n8n` を両方 `suspend: true`、両方に `flux.takutk.com/activation-blocked: "true"`。
