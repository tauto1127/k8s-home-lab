...
| `clusters/home/packages/trek/*`, `clusters/home/flux-system/sync.yaml` | migration-pending | TREK 4.2.1 preparation package。`trekActivation`のpreparation/config-active/app-active state machineで外側Kustomizationと内側HelmReleaseの所有境界・marker・suspendを固定し、ESO controller/configに依存する。各transitionは別Git PRで行う。 |
